<script lang="ts">
	...
	$: if (pickedDay == 'All' && pickedLocation == 'All') {
		...
		displayedSchedules = schedules.filter((schedule) => schedule.day.includes(pickedDay));
...
{#if displayedSchedules && displayedSchedules.length}
	{#each displayedSchedules as schedule}
		<div class="grid p-6 mb-4 border-4 rounded shadow bg-zinc-300 border-zinc-900 grow ">
			<h2 class="pb-2">{schedule.AO.name}</h2>
			<div class="flex flex-col gap-2 ">
				<div class="flex">
					<p>Days:</p>
					{#each schedule.day as day}
						<span class="px-2 underline underline-offset-2 font-bold">{day}</span>
					{/each}
				</div>
